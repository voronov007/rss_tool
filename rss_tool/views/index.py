...
from rss_tool.models import Feed, Comment
...
class IndexView(View):
    ...
    def get(self, request, *args, **kwargs):
        feeds = Feed.objects.select_related(
            "channel"
        ).prefetch_related(
            "comments__author"
        ).filter(
            channel__user_id=request.user.pk
        ).order_by("id")
        ...
        if has_previous:
            output["pagination"]["previous_page_number"] = feeds_per_page.previous_page_number()

        if has_next:
            output["pagination"]["next_page_number"] = feeds_per_page.next_page_number()
